...
export function ConfirmResultForm({ fixtureId, clubAId, clubBId, clubAName, clubBName, defaultA = 0, defaultB = 0 }: Props) {
  ...
  return (
    <form action={action} className="flex flex-wrap items-end gap-3 mt-4 pt-4 border-t border-border">
      ...
      <div className="flex items-center gap-2">
        <label className="text-xs text-muted w-28 text-right truncate">{clubAName}</label>
        <input
          name="score_a"
          type="number"
          min="0"
          defaultValue={defaultA}
          className="w-14 border border-border bg-white text-navy text-sm px-2 py-1.5 rounded focus:outline-none focus:border-cobalt transition-colors text-center"
        />
        ...
        <input
          ...
          className="w-14 border border-border bg-white text-navy text-sm px-2 py-1.5 rounded focus:outline-none focus:border-cobalt transition-colors text-center"
        />
        <label className="text-xs text-muted w-28 truncate">{clubBName}</label>
      </div>
