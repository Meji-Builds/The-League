...
import { updateCompetitionBanner } from "./actions";
...
export function BannerUploadForm({ competitionId, currentBannerUrl }: Props) {
  const [state, formAction, isPending] = useActionState(updateCompetitionBanner, null);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (file && file.size > 0) {
      setUploading(true);
      const uploaded = await directUpload(file, `competitions/${competitionId}`);
      setUploading(false);
      if (!uploaded) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-3 pt-3 border-t border-white/6">
      <p className="text-xs font-semibold text-white/70 uppercase tracking-wide mb-2">Banner Image</p>
