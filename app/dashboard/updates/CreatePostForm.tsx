...
export function CreatePostForm({ clubId }: { clubId: string }) {
  ...
  return (
    <div className="border border-white/6 bg-card p-5">
      <h2 className="font-bold text-white text-base mb-4">Post a Club Update</h2>

      {state && "success" in state && (
        <div className="bg-success/5 border border-success/30 text-success text-xs px-3 py-2 mb-4">
      ...
      <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full border border-white/10 bg-navy/50 text-white text-sm px-3 py-2 focus:outline-none focus:border-cobalt transition-colors placeholder:text-white/20 resize-y"
          />
        </div>

        <div>
          <label className="block text-white/70 text-xs font-semibold mb-1 uppercase tracking-wide">
            Image <span className="text-white/40 font-normal normal-case">(optional)</span>
          </label>
          <input
            ref={fileRef}
            name="image"
            type="file"
            accept="image/*"
            className="w-full text-sm text-white/50 file:mr-3 file:text-xs file:font-semibold file:bg-navy file:text-gold file:border-0 file:px-3 file:py-1.5 file:cursor-pointer hover:file:bg-navy/80"
          />
        </div>
