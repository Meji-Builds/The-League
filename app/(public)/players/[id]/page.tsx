...
function ClubAvatar({ club, size = 6 }: { club: { name: string; logo_url?: string | null; logo_status?: string | null } | null; size?: number }) {
  const name    = club?.name ?? "?";
  const color   = avatarColor(name);
  const hasLogo = club?.logo_url && club.logo_status === "approved";
  const sz      = size === 6 ? "w-6 h-6" : size === 7 ? "w-7 h-7" : size === 8 ? "w-8 h-8" : "w-6 h-6";
  return (
...
export default async function PlayerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: fixtures } = await db
    .from("fixtures")
    .select("id, stage, status, scheduled_at, confirmed_score, club_a:clubs!fixtures_club_a_id_fkey(id, name, slug, logo_url, logo_status), club_b:clubs!fixtures_club_b_id_fkey(id, name, slug, logo_url, logo_status), competition:competitions(name, slug)")
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 flex-1 justify-end min-w-0">
                        ...
                        <ClubAvatar club={myClub ?? null} size={6} />
                      </div>
